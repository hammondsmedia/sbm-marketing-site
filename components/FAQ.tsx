...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all duration-300 ${open ? 'border-blue/40 shadow-md shadow-blue/8' : 'border-bg-border hover:border-blue/30'}`}>
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between gap-4 p-6 text-left bg-white hover:bg-bg-surface transition-colors"
      >
        <span className="text-sm md:text-base font-semibold leading-snug" style={{ color: '#1a2b4a' }}>{q}</span>
        <span
          className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
            open ? 'rotate-45 bg-blue-light border border-blue/30' : 'border border-gray-200'
          }`}
        >
          <svg
            className={`w-4 h-4 transition-colors duration-300 ${open ? 'text-blue-dark' : 'text-gray-400'}`}
